...
void ttk::DiscreteMorseSandwichMPI::updateMaxBoundary(
  ...
  if(!globalBoundary.empty()) {
    ...
    for(const auto max : globalBoundary) {
      if(max.proc_ != rank) {
        /*if (s2 == 1282){
          printMsg("Send update max boundary of "+std::to_string(s2)+" to
...
void ttk::DiscreteMorseSandwichMPI::updateLocalBoundary(
  ...
  /*if (s2.gid_ == 1282){
    printMsg("Send update local boundary of "+std::to_string(s2.gid_)+" to
  "+std::to_string(rank));
  } */
#pragma omp atomic read
  tempCurrentBuffer = currentBuffer_;
...
void ttk::DiscreteMorseSandwichMPI::updateMergedBoundary(
  ...
  if(!globalBoundary.empty()) {
    ...
    for(const auto max : globalBoundary) {
      sendBoundaryBufferLock_[max.proc_].lock();
      /*if (s2.gid_ == 1282){
        printMsg("Send merge boundary order of "+std::to_string(s2.gid_)+" to
      "+std::to_string(max.proc_));
      } */
#pragma omp atomic read
      tempCurrentBuffer = currentBuffer_;
...
#pragma omp atomic update
    ...
    /*if (triangulation.getEdgeGlobalId(tau) == 657 ||
    triangulation.getEdgeGlobalId(tau) == 673){ kill(getpid(), SIGINT);
    }*/
    //}
    ...
    if(pTau == -1) {
      ...
      if(globalToLocalSaddle1_.end() != it) {
        ...
        if(tooFar) {
          ...
          /*int threadNumber = omp_get_thread_num();
          sendComputeBuffer_[threadNumber][globMax.proc_].emplace_back(s2.gid_);
...
#pragma omp parallel for reduction(merge : saddles2Gid) schedule(static)
  ...
  this->messageSize_ = saddle2Number * 0.1;
...
#pragma omp parallel num_threads(threadNumber_) firstprivate(onBoundary)    \
  shared(s1Locks, s2Locks, s2GlobalBoundaries, s2LocalBoundaries, saddles1, \
         saddles2, edgeTrianglePartner)
  {
...
#pragma omp atomic read
        ...
        for(int i = 0; i < ttk::MPIsize_; i++) {
          ...
          if(i != ttk::MPIrank_) {
            ...
            localSentMessageNumber
              += sendMessageSize[i][0] + sendMessageSize[i][1];
          }
        }
...
#pragma omp task firstprivate(lid)                                            \
  shared(s2GlobalBoundaries, s2LocalBoundaries, edgeTrianglePartner, s1Locks, \
      ...
      if(totalFinishedPropagationCounter != globalSaddle2Counter_) {
        printErr("PROBLEM: total: "
                 + std::to_string(totalFinishedPropagationCounter)
                 + ", global: " + std::to_string(globalSaddle2Counter_));
      }
      if(ttk::MPIrank_ == 0)
        printMsg("Communication rounds: " + std::to_string(count));
    }
    }
  }
