...
void ttk::DiscreteMorseSandwich::tripletsToPersistencePairs(
  ...
  const auto getClosestRep
    ...
    order("+std::to_string(saddlesOrder[repVect[i][1]])+"); ";
      ...
      printMsg("reps for v: "+std::to_string(v)+ " and sv:
    "+std::to_string(sv)+", order("+std::to_string(saddlesOrder[sv])+"): "+s);
    }  */
    ...
    /* if(!((saddlesOrder[rep[1]] < saddlesOrder[sv]) == increasing)) {
       if (sv == 300)
           printMsg("leaving early getClosestRep: "+std::to_string(rep[0])+",
     "+std::to_string(rep[1])
           +", " +std::to_string(repVect[i][0])+",
     "+std::to_string(repVect[i][1])); return std::array<ttk::SimplexId,
     3>{repVect[i][0], repVect[i][1], i};
     }
     i--;
     rep = repVect[i];*/
    while(i > 0
  ...
  const auto getRep
    ...
    return std::array<ttk::SimplexId, 4>{v, s, index, vprec};
  };
  ...
  const auto findIncorrectSaddle
    = [this, &saddleToPairedExtrema](
        const ttk::SimplexId r,
        std::vector<std::array<ttk::SimplexId, 2>> &repVect) -> ttk::SimplexId {
    auto it = repVect.rbegin();
    // std::string s = "";
    // s += "("+std::to_string((*it)[0])+","+std::to_string((*it)[1])+") ";
    while((*it)[1] != -1 && saddleToPairedExtrema[(*it)[1]] != r) {
      it++;
      // s += "("+std::to_string((*it)[0])+","+std::to_string((*it)[1])+") ";
    }
    /*if (r == 127 || r == 212 || r == 337)
      printMsg(s+" for repVect of size: "+std::to_string(repVect.size()));*/
    return (*it)[1];
  };
  ...
  const std::function<int(tripletType)> processTriplet
    = [this, &increasing, &pairedExtrema, &pairedSaddles,
       ...
       &PCCounter, &findIncorrectSaddle](tripletType t) -> int {
    const auto sv = t[0];
    /*if(sv == 300 || sv == 18) {
    ...
    auto r1 = rep1[0];
    auto vprec1 = rep1[3];
    auto s1 = findIncorrectSaddle(r1, reps[r1]);
    // auto s1 = rep1[1];
    auto index1 = rep1[2];
    ...
    if(t[2] < 0) {
      /*if(sv == 300 || sv == 18 || sv == 854) {
        printMsg("sv for t[2]=-1: " + std::to_string(sv) + ", order("
      ...
      if(!pairedR1 && !pairedSaddles[sv]) {
        ...
        // maximum
        if(isR1Invalid) {
          // s1 = findIncorrectSaddle(r1, reps[r1]);
          /*if (saddleToPairedExtrema[s1] == 231)
            printErr("Remove pair "+std::to_string(s1)+" with
            "+std::to_string(saddleToPairedExtrema[s1])+" by
            "+std::to_string(sv));*/
          removePair(s1);
        }
        addPair(sv, r1);
    ...
    auto r2 = rep2[0];
    auto vprec2 = rep2[3];
    auto s2 = findIncorrectSaddle(r2, reps[r2]);
    ;
    // auto s2 = rep2[1];
    auto index2 = rep2[2];
    ...
    /*if(sv == 300 || sv == 18 || sv == 854) {
      printMsg(
        ...
        + ", s2: " + std::to_string(s2));
      //printMsg("is R1 paired: "+std::to_string(pairedR1)+", is r1 invalid:
    "+std::to_string(isR1Invalid));
      //printMsg("is R2 paired: "+std::to_string(pairedR2)+", is r2 invalid:
    "+std::to_string(isR2Invalid));
    }    */
    if(r1 != r2) {
      ...
      /*if(sv == 584) {
        printMsg(
          "sv: " + std::to_string(sv) + ", order("
          + std::to_string(saddlesOrder[sv]) + "), t[1]: " +
      std::to_string(t[1])
          + ", r1: " + std::to_string(r1) + ", s1: " + std::to_string(s1)
          + ", t[2]: " + std::to_string(t[2]) + ", r2: " + std::to_string(r2)
          + ", s2: " + std::to_string(s2));
      ...
      if(!pairedR1) {
        ...
        /*if (sv == 18){
          printMsg("remove "+std::to_string(s1)+" because r1 is invalid:
        "+std::to_string(isR1Invalid));
        }*/
        addPair(sv, r1);
        svToR[sv] = std::array<ttk::SimplexId, 2>{t[1], t[2]};
        ...
        if(isR1Invalid) {
          ...
          1)+", "+s);*/
          /*if (s1 == 300 || s1 == 18){
            printMsg("Start rerun for "+std::to_string(s1)+" from
          "+std::to_string(sv));
          }*/
          rerunCounter++;
          return processTriplet(tripletType{s1, svToR[s1][0], svToR[s1][1]});
